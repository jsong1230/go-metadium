...
func (d *Downloader) Progress() ethereum.SyncProgress {
	...
	case d.blockchain != nil && mode == SnapSync:
		current = d.blockchain.CurrentFastBlock().Number.Uint64()
...
func (d *Downloader) findAncestor(p *peerConnection, remoteHeader *types.Header) (uint64, error) {
	...
	case SnapSync:
		localHeight = d.blockchain.CurrentFastBlock().Number.Uint64()
	default:
		...
		if skeleton {
			...
			if n := len(headers); n > 0 {
				...
				if mode == LightSync {
					...
					head = d.blockchain.CurrentFastBlock().Number.Uint64()
