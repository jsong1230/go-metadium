...
[[ -f genesis.json ]] || { echo "[ERROR] genesis.json 없음. setup.sh를 먼저 실행하세요."; exit 1; }

log "=== PoA 프라이빗 네트워크 시작 ==="
docker compose up -d
...
log ""
log "유용한 명령:"
log "  블록 모니터링: watch -n2 'curl -sf -X POST -H \"Content-Type: application/json\" --data \"{\\\"jsonrpc\\\":\\\"2.0\\\",\\\"method\\\":\\\"eth_blockNumber\\\",\\\"params\\\":[],\\\"id\\\":1}\" http://localhost:8545 | python3 -c \"import sys,json; d=json.load(sys.stdin); print(int(d[\\\"result\\\"],16))\"'"
log "  node1 로그:   docker logs -f gmet-poa-node1"
log "  중지:         ./stop.sh"
