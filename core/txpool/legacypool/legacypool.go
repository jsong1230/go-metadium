...
func New(config Config, chain BlockChain) *LegacyPool {
	...
	// Metadium: initialize sender resolver and TRS
	pool.senderResolver = NewSenderResolver(8, 1024000)
	pool.trsListMap = make(map[common.Address]bool)

	return pool
}

// Filter returns whether the given transaction can be consumed by the legacy
// pool, specifically, whether it is a Legacy, AccessList, Dynamic or
// Metadium FeeDelegateDynamicFee (Type 22) transaction.
...
func (pool *LegacyPool) validateTxBasics(tx *types.Transaction, local bool) error {
	opts := &txpool.ValidationOptions{
		...
		Accept: 0 |
			...
			1<<types.FeeDelegateDynamicFeeTxType, // Metadium Type 22
		MaxSize: txMaxSize,
	...
	// Metadium: validate fee delegation (Type 22) specific fields
