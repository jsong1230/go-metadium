...
type ExecutionResult struct {
	...
	Fee         *big.Int // Transaction fee (Metadium)
...
type Message struct {
	...
	// FeePayer is set for Metadium Type 22 (FeeDelegateDynamicFeeTx) transactions.
	// When non-nil, gas fees are paid by FeePayer instead of From.
...
func (st *StateTransition) buyGas() error {
	...
	if st.evm.ChainConfig().IsCancun(st.evm.Context.BlockNumber, st.evm.Context.Time) ||
		st.evm.ChainConfig().IsCamellia(st.evm.Context.BlockNumber) {
	...
	if st.msg.FeePayer != nil {
		...
		if feePayer == st.msg.From {
			...
			if overflow {
				return fmt.Errorf("%w: address %v required balance exceeds 256 bits", ErrInsufficientFunds, feePayer.Hex())
			}
			if have, want := st.state.GetBalance(feePayer), balanceCheckU256; have.Cmp(want) < 0 {
				return ErrFeePayerInsufficientFunds
			}
		} else {
			mgvalCheckU256, overflow := uint256.FromBig(mgval)
			if overflow {
				return fmt.Errorf("%w: address %v required balance exceeds 256 bits", ErrInsufficientFunds, feePayer.Hex())
			}
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	if st.evm.Config.NoBaseFee && msg.GasFeeCap.Sign() == 0 && msg.GasTipCap.Sign() == 0 {
		...
		bigFee = fee.ToBig()
		// In Metadium, block reward and fees are combined and distributed via governance.
		if metaminer.IsPoW() {
...
func (st *StateTransition) refundGas(refundQuotient uint64) uint64 {
	...
	// Metadium fee delegation: return gas refund to fee payer
	if st.msg.FeePayer != nil {
