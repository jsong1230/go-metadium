...
func testRepair(t *testing.T, tt *rewindTest, snapshots bool) {
	...
	if head := newChain.CurrentFastBlock(); head.Number.Uint64() != tt.expHeadFastBlock {
		t.Errorf("Head fast block mismatch: have %d, want %d", head.Number.Uint64(), tt.expHeadFastBlock)
	}
	if head := newChain.CurrentBlock(); head.Number.Uint64() != tt.expHeadBlock {
		t.Errorf("Head block mismatch: have %d, want %d", head.Number.Uint64(), tt.expHeadBlock)
	}
...
func TestIssue23496(t *testing.T) {
	...
	if head := chain.CurrentFastBlock(); head.Number.Uint64() != uint64(4) {
		t.Errorf("Head fast block mismatch: have %d, want %d", head.Number.Uint64(), uint64(4))
	}
	if head := chain.CurrentBlock(); head.Number.Uint64() != uint64(1) {
		t.Errorf("Head block mismatch: have %d, want %d", head.Number.Uint64(), uint64(1))
	}
	...
	if head := chain.CurrentBlock(); head.Number.Uint64() != uint64(4) {
		t.Errorf("Head block mismatch: have %d, want %d", head.Number.Uint64(), uint64(4))
	}
